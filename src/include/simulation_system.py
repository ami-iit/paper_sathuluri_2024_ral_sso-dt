...
def run_walking_sim(
    ...
    demux2 = builder.AddSystem(Demultiplexer([20, 20]))  
    ...
    if add_base_noise != 0.0:
        noise_adder = builder.AddSystem(
            NoiseAdder(
                ...
                noise_type="uniform",
                noise_params={
                    "min": np.array([-abs(add_base_noise)] * 3),
                    "max": np.array([abs(add_base_noise)] * 3),
                },
            )
        )
        builder.Connect(
    ...
    if add_root_disturbance != 0.0:
        disturbance_sys = builder.AddSystem(
            DisturbanceSystem(
                ...
                force_type="uniform",
                force_params={
                    "min": np.array([0] * 3 + [-abs(add_root_disturbance)] * 3),
                    "max": np.array([0] * 3 + [abs(add_root_disturbance)] * 3),
                },
                time=5,
                duration=0.1,
                meshcat=meshcat,
