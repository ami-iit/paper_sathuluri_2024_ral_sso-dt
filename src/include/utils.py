...
params_folder = (
    Path(__file__).resolve().parent.parent.parent.parent.__str__()
    + "/robots-configuration/ergoCubSN000/hardware/mechanicals/"
)
...
# %%
def extract_expt_data_dict(filename):
...
def load_solution(filetag):
    with open(f"../optimisation_logs/{filetag}_opt_progress.pkl", "rb") as f:
        all_sols = pickle.load(f)
    all_res = np.array([all_sols[k]["res"] for k in all_sols.keys()])
    all_res = all_res.flatten()
    min_idx = np.where(all_res.flatten() == np.min(all_res.flatten()))[0][-1]
    all_x = np.array([all_sols[k]["x"] for k in all_sols.keys()])
    all_x = all_x.reshape(-1, all_x.shape[-1])
    selected_point_optimum = all_x[min_idx]
    print("Minimum objective function value:", np.min(all_res))
    return selected_point_optimum


def plot_optimisation_progress(filetag, save_fig=False):
    with open(f"../optimisation_logs/{filetag}_opt_progress.pkl", "rb") as f:
        all_sols = pickle.load(f)
    gens = list(all_sols.keys())
    fvals = [np.min(all_sols[k]["res"]) for k in gens]
    xs = [all_sols[k]["x"][np.argmin(all_sols[k]["res"])] for k in gens]
    fig, ax = plt.subplots()
    ...
    if save_fig:
        plt.savefig(
            f"../assets/opt_prog_{filetag.split('/')[-1]}.pdf",
            format="pdf",
            dpi=300,
            bbox_inches="tight",
        )
